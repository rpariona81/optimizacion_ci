<div class="page-header">
    ...
    <?php
    echo anchor('encuestacsc/todos', 'TODOS', array('id' => 'vervalidos', 'class' => 'btn btn-warning btn-sm')) . " ";
    echo anchor('encuestacsc/validos', 'VALIDOS', array('id' => 'vervalidos', 'class' => 'btn btn-success btn-sm')) . " ";
    echo anchor('encuestacsc/pendientes', 'PENDIENTES', array('id' => 'verpendientes', 'class' => 'btn btn-info btn-sm')) . " ";
    echo anchor('encuestacsc/eliminados', 'ELIMINADOS', array('id' => 'vereliminados', 'class' => 'btn btn-danger btn-sm')) . " ";
    echo '<div style="float:right; margin-right:10px;">';
    echo anchor('encuestacsc/exporta_csc_usu_ext_norte2', 'Descarga LIMA NORTE 2', array('class' => 'btn btn-primary btn-sm'));
    echo '</div>';
    ?>
</div>
        ...
        <tbody>
            <?php foreach ($cuestionarios as $item): ?>
                <tr class="todo-list">
                    ...
                    <td><?= utf8_encode($item->ENCUESTADO) ?></td>
                    <td ><?= $item->RUC ?></td>
                    <td ><?= $item->DISTRITO ?></td>
                    <td ><?= $item->ESTADO ?></td>
                    <td ><?= $item->FECHA ?></td>
                    <td class="text-center" >
                        ...
                        <?php
                        if ($item->ACTIVO == '0') {
                            echo anchor('encuestacsc/activar/' . $item->ID, 'Habilitar <svg class="glyph stroked trash"><use xlink:href="#stroked-trash"></use></svg>', array('class' => 'trash btn btn-danger btn-sm'));
                        }
                        ?>
